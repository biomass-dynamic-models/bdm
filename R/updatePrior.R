...
updatePrior.bdm <-  function(object, prior, ...) {
    ...
    if (is.list(prior)) {
        ...
        if (length(lognormal_par) == 0) {
            ...
            # assign
            lognormal_par <- list('E[log(x)]' = mu, 'SD[log(x)]' = sigma, 'E[x]' = theta, 'VAR[x]' = nu, 'CV[x]' = cv)
            
        } 
        
        logmu    <- signif(lognormal_par[['E[log(x)]']],3)
        logsigma <- signif(lognormal_par[['SD[log(x)]']],3)
