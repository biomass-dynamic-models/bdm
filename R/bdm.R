...
#' @param model_code model code as a character string
...
#' 
#' @export
bdm <- function(path, model_code, model_name = '') {
    if (!missing(path)) { 
        model_name <- ifelse(model_name == '', sub("\\.[^.]*$", "", basename(path)), model_name)
        new('bdm', path = path, model_name = model_name)
    } else { 
        if (!missing(model_code)) { 
            model_name <- ifelse(model_name == '', deparse(substitute(model_code)), model_name)
            if (substr(model_name, start = 0, stop = 1) == "\"") 
                stop('please supply a model_name\n')
            new('bdm', model_code = model_code, model_name = model_name)
        } else {
            model_name <- ifelse(model_name == '', 'default', model_name)
            new('bdm', model_code = bdm_default, model_name = model_name)
        }
    }
}
#{{{
